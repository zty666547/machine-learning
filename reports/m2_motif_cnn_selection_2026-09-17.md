...
阶段记录日期：2026 年 9 月 14 日。目标为在固定 90% 相似性聚类划分上，以 `log10(强度)` 训练位置感知 CNN。除表中列出的卷积宽度和过滤器数量外，其余设置一致：五个位置区间、64 单元 ReLU 预测头、学习率 0.001、权重衰减 0.0001、随机种子 20260912 和早停。

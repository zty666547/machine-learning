#!/usr/bin/env python3
"""Compare VAE KL weights across seeds using validation and generation metrics."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output", default="reports/m3_vae_beta_stability.json")
    parser.add_argument("--beta-values", default="0.05,0.10,0.20")
...
def main() -> None:
    ...
    logger = configure_logging("tune_vae_beta_stability", REPOSITORY_ROOT / config["logging"]["directory"])
    ...
    beta_values = [float(value) for value in args.beta_values.split(",")]
    seeds = [int(value) for value in args.seeds.split(",")]
    trials = []
    for beta in beta_values:
        for seed in seeds:
            model = ConditionalSequenceVAE(beta=beta, epochs=args.epochs, seed=seed)
            ...
            logger.info("VAE beta stability trial: %s", record)
    summary = []
    for beta in beta_values:
        matching = [record for record in trials if record["beta"] == beta]
        summary.append({
            "beta": beta,
            "mean_validation_loss": float(np.mean([record["validation_loss"] for record in matching])),
            "mean_target_gc_pearson": float(np.mean([record["target_gc_pearson"] for record in matching])),
            "std_target_gc_pearson": float(np.std([record["target_gc_pearson"] for record in matching])),
    ...
    result = {"latent_size": 16, "epochs": args.epochs, "samples_per_target": args.samples_per_target, "trials": trials, "summary": summary, "selection_rule": "lowest mean 3-mer JS, then strongest mean absolute GC response correlation"}
